...
def parse_batch_file(filepath: Path) -> list[list]:
    ...
    Supports both single-row and multi-row INSERT … VALUES (…), (…), … ; syntax.
    """
    content = filepath.read_text(encoding="utf-8")
    m = re.search(r"\bVALUES\b\s*(.*?)\s*;", content, re.DOTALL | re.IGNORECASE)
    if not m:
        return []
    ...
    for idx, ch in enumerate(block):
        if ch == "(" and depth == 0:
            ...
            depth = 0
            rows.append(_parse_row_values(block[start:idx]))
